...
\begin{enumerate}
  ...
  \item \textbf{Punctuation removal} — keep only letters, digits, whitespace, apostrophes, and hyphens. This preserves contractions (``don't'') and compound terms (``re-entry'') while removing most punctuation. Punctuation patterns (e.g., ``!!!'' in spam) could be informative, but at this stage we simplify to pure word features.
...
The V2 Merged strategy is also evaluated during threshold tuning alongside Balanced and Unbalanced. Although V2 Merged provides the largest and most diverse training set, the winning configuration in this project was the Balanced strategy, as determined by the validation-based threshold selection process.
...
Logistic Regression & Discriminative (batch GD) & Binary Cross-Entropy + L2 & 120 epochs, lr=0.800\\
...
Logistic regression estimates $P(\text{spam}) = \sigma(X\mathbf{w} + b)$ and minimizes binary cross-entropy via batch gradient descent. The learning rate of 0.800 is unusually high for batch gradient descent but works here because the sigmoid gradient is bounded and smooth — it never explodes. The L2 penalty of $10^{-4}$ prevents weight divergence in the 138,942-dimensional feature space: without it, weights for rare features would grow unboundedly on samples where those features perfectly predict the label.

\begin{pythoncode}[caption={Logistic Regression: Batch Gradient Descent with Binary Cross-Entropy}]
...
\subsubsection{Model Comparison at Default Operating Points}

All four models (centroid, NB, LR, SVM) are evaluated on the \textbf{same} 1,978-row test set at their respective default operating points: probability threshold 0.500 for NB and LR (class with highest posterior probability); decision score threshold 0 for SVM (positive score = spam); and cosine similarity comparison for the nearest centroid baseline. At these operating points, the models achieve the following accuracies:
...
\begin{itemize}
  ...
  \item \textbf{Logistic Regression:} 0.918 — below the centroid baseline, revealing that the default probability threshold is poorly calibrated for this imbalanced, high-dimensional problem. LR's probability estimates are compressed toward 0.500, so the hard cutoff misclassifies a significant fraction of borderline cases. After threshold tuning, LR performance improves substantially.
...
\begin{itemize}
  ...
  \item A model with elevated FPs is too ``aggressive'' at the default operating point, classifying borderline ham as spam. A model with elevated FNs is too ``conservative,'' letting borderline spam through. The optimal threshold shifts each model along this spectrum without retraining weights.
...
Every scratch prediction is compared against sklearn's reference implementation to catch implementation bugs. The disagreement rate is the fraction of samples where scratch and sklearn predictions differ:

\begin{itemize}
  \item \textbf{Multinomial NB:} Disagreement = 0.000 (agreement = 100.000\%). The MLE counting implementation is deterministic and has no optimizer variance, so perfect agreement is expected.
  \item \textbf{Logistic Regression:} Disagreement = 0.043 (agreement = 95.700\%). The 4.300\% disagreement stems from sklearn using L-BFGS (a quasi-Newton second-order solver) while our implementation uses first-order batch gradient descent. Both solvers converge to near-optimal regions, but the exact weight vectors differ, and the 0.500 threshold amplifies these differences for borderline cases.
  \item \textbf{Linear SVM:} Disagreement = 0.037 (agreement = 96.300\%). Similar to LR, sklearn uses SMO (a coordinate-descent solver) while ours uses sub-gradient descent. The 3.700\% disagreement reflects solver-specific convergence paths rather than implementation bugs.
\end{itemize}

Disagreement below 5.000\% for the two iterative models confirms that our gradient computations and update rules are correct — the residual gap is solver architecture, not algorithmic error. Perfect agreement for NB validates the deterministic counting implementation.

\subsection{Threshold Tuning (Gold Standard Optimization)}

This is the most critical optimization step. The model weights are \textit{not} retrained — only the decision threshold $\tau$ is tuned. The insight is that a model's raw scores contain far more information than any single fixed cutoff reveals; choosing the right threshold extracts that information.
...
TARGET_FPR = 0.010   # at most 1% of ham misclassified
...
\begin{itemize}
  ...
  \item \textbf{2 feature sets:} Text only (TF-IDF on cleaned subject and body text combined — `data["text"] = data[["subject", "body"]].fillna("").agg(" ".join, axis=1)` joins both fields before cleaning) and Text + Metadata (appending sender, source family, and subject line to the text before vectorization). Subject lines are rich in discriminative signal (\texttt{Subject: Urgent}, \texttt{Subject: Limited offer}), and joining them with the body ensures that subject-only indicators are not lost.
  \item \textbf{Models:} For the Text-only feature set, all three models (NB, LR, SVM) are evaluated. For the Text + Metadata set, only the Linear SVM is evaluated (at two regularization strengths: C=0.5 and C=1.0), because metadata features are high-dimensional and SVM's margin formulation handles them most robustly. Each produces different score distributions, and the optimal threshold varies by model.
\end{itemize}

This yields exactly $3 \times (3 + 2) = 15$ combinations. For each combination, candidate thresholds are generated automatically from the unique prediction scores via sklearn's roc_curve() function. For each combination, the threshold that maximizes TPR while keeping validation FPR $\le$ 0.010 is selected.
...
\begin{resultbox}
The 15-combination threshold sweep identified the optimal configuration:
\begin{itemize}
  ...
  \item \textbf{Strategy:} Balanced (Text + Metadata features, designated Meta C1) — the training pool was downsampled to an equal number of ham and spam emails, eliminating the class-imbalance bias in the loss function. The supplementary ham data (V2 Merged) was evaluated as an alternative but the Balanced + Meta SVM configuration achieved the best validation operating point.
  \item \textbf{Selected threshold:} $\tau = 0.089$ — slightly above the SVM default decision threshold of 0, meaning the classifier is slightly more conservative when assigning the spam label. The Linear SVM outputs decision scores (not probabilities): ham emails generally receive negative scores, whereas spam emails generally receive positive scores.
  \item \textbf{Validation performance:} TPR = 0.999, FPR = 0.006 at the FPR limit of 0.010 — exceeding the gold standard requirements on held-out validation data.
  \item \textbf{Final test performance:} TPR = 0.998, FPR = 0.009 on the 1,978-row test set. The corresponding accuracy, precision, recall, and F1-score at the selected threshold $\tau = 0.089$ are: accuracy = 0.994, precision = 0.989, recall = 0.998, F1-score = 0.993.
  \item \textbf{Gold standard:} \textbf{ACHIEVED} — TPR $\ge$ 0.990 AND FPR $\le$ 0.010.
\end{itemize}

The key insight: \textbf{threshold tuning matters more than model choice.} At the default operating points, the four models (centroid, NB using probability 0.500, LR using probability 0.500, SVM using decision score 0) cluster within a 0.051 accuracy range (0.918--0.969). After threshold tuning, the gap between default and optimized performance dwarfs the gap between models. The bottleneck is not the model architecture — it is the decision rule. By shifting $\tau$ from the SVM default of 0 to 0.089, we exploit the fact that the SVM's decision scores for ham and spam are well-separated: ham emails receive negative scores, while spam emails receive positive scores. This wide separation creates a ``safe zone'' between 0.089 and approximately 0.900 where FPR stays below 0.010 while TPR stays above 0.990.
\end{resultbox}

\begin{figure}[H]\centering
\includegraphics[width=0.70\textwidth]{figures/lab01/confusion_matrix.png}
\caption{Confusion matrix for the Balanced (Text + Metadata) Linear SVM (Meta C1) configuration at the selected decision threshold $\tau = 0.089$. The overwhelming majority of predictions land on the diagonal (correct classifications), with only a handful of off-diagonal entries (2 false negatives, 10 false positives out of 1,978 test emails).}
\end{figure}
...
\begin{notebox}
\textbf{Threshold tuning is more impactful than model choice, but model choice determines how tunable the threshold is.} At the default operating points, all models show broadly similar behavior because the default cutoffs (probability 0.500 for NB and LR, decision score 0 for SVM) are arbitrary — they were not designed for this dataset or this metric. After tuning, the performance gap is determined by how cleanly each model's score distributions separate. The Linear SVM produces the widest separation because the hinge loss explicitly optimizes for margin: it pushes spam decision scores toward $+\infty$ and ham scores toward $-\infty$, creating room to place a threshold. Logistic regression, by contrast, produces probability estimates that can be compressed or miscalibrated, leaving less room for threshold adjustment. NB's generative approach produces well-separated probability scores but is sensitive to the independence assumption — when word co-occurrence patterns differ between training and test sources, NB's calibrated probabilities degrade.

The Balanced strategy with Text + Metadata features (Meta C1) achieves the best TPR at the 0.010 FPR constraint. Despite discarding roughly half the ham data, the balanced training pool eliminates the loss-function bias that would otherwise favor the majority class. The metadata features (sender, source, subject) provide explicit provenance that the SVM can learn to discount, rather than relying on implicit source signals encoded in vocabulary patterns. The V2 Merged strategy was also competitive, but the Balanced configuration achieved superior TPR at the required FPR limit on validation, demonstrating that careful threshold selection on a balanced pool can outperform simply adding more data.
\end{notebox}
