...
\midrule
\texttt{ngram\_range} & (1, 2) & Bigrams capture phrases like ``free money'' beyond individual words — a bigram can be discriminative even when its constituent unigrams are not\\
...
\texttt{sublinear\_tf} & True & Prevents high-frequency tokens from dominating — a word appearing 100$\times$ in one email is not 100$\times$ more important\\
\bottomrule
\end{tabular}
