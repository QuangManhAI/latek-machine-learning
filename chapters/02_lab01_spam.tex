...
\begin{itemize}
  ...
  \item \textbf{Logistic Regression:} Disagreement = 0.043 (agreement = 95.700\%). The 4.300\% disagreement stems from sklearn using L-BFGS (a quasi-Newton second-order solver) while our implementation uses first-order SGD. Both solvers converge to near-optimal regions, but the exact weight vectors differ, and the 0.500 threshold amplifies these differences for borderline cases.
  \item \textbf{Linear SVM:} Disagreement = 0.037 (agreement = 96.300\%). Similar to LR, sklearn uses SMO (a coordinate-descent solver) while ours uses sub-gradient SGD. The 3.700\% disagreement reflects solver-specific convergence paths rather than implementation bugs.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Final test performance:} TPR = 0.998, FPR = 0.009 on the 1,978-row test set. The corresponding accuracy, precision, recall, and F1-score at the selected threshold $\tau = 0.089$ are: accuracy = 0.994, precision = 0.991, recall = 0.998, F1-score = 0.995.
