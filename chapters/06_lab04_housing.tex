...
\subsection{Dataset}
\begin{itemize}
  \item \textbf{Source:} California Housing (1990 census), 20,640 rows, 10 columns (8 numeric predictors, 1 categorical predictor, 1 numeric target).
...
\texttt{ISLAND}      & 5     & Negligible sample size — kept as is despite sparsity\\
\bottomrule
\end{tabular}
\caption{Ocean proximity category counts and characteristics}
\end{table}

The ISLAND category contains only 5 samples, which is too sparse to learn a reliable coefficient. However, the notebook retains all 5 categories in the one-hot encoding, as merging would risk masking any extreme price signal present in these few blocks. The price disparity between INLAND (cheapest) and non-INLAND categories is substantial enough to consider an \texttt{is\_inland} binary feature as a high-signal engineering candidate. The price disparity between INLAND (cheapest) and non-INLAND categories is substantial enough to consider an \texttt{is\_inland} binary feature as a high-signal engineering candidate, though the full one-hot encoding is used to preserve granularity.
...
\begin{enumerate}
  ...
  \item \textbf{One-hot encoding:} \texttt{OneHotEncoder(sparse\_output=False)} converts the 5 \texttt{ocean\_proximity} categories into 5 binary indicator columns (no reference category dropped; all categories are retained as explicit columns).
...
\textbf{Output dimensions:} $X_{\text{train}}$ becomes $(16512, 16)$ and $X_{\text{test}}$ becomes $(4128, 16)$. The 16 columns correspond to: 8 original numeric features + 3 engineered ratio features (\texttt{rooms\_per\_household}, \texttt{population\_per\_household}, \texttt{bedrooms\_per\_room}) + 5 one-hot encoded ocean proximity categories (all 5 categories retained).
